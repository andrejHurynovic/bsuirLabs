    ...
    internal class SyncTcpServer : IDisposable
    {
        ...
        public void Start()
        {
            ...
            while (true)
            {
                ...
                _mainSocket.BeginAccept(OnNewConnection, this);
                ...
                //if (TempFile.IsExist())
                //{
                //    ConsoleHelpers.LogWarning("Some file is not loaded! Trying to restore download!", typeof(SyncTcpServer).ToString(), true);
                //    RestoreUpload();
                //}
                //else
                //{
        ...
        {
            _connections.Add(new Connection(_mainSocket.EndAccept(ar), BufferSize));
#if CONSOLE_LOGGING_NOTIFIES || CONSOLE_LOGGING_ALL
            ConsoleHelpers.LogNotify($"New connection: {_connections.Last().Socket.RemoteEndPoint}", typeof(SyncTcpServer).ToString(), true);
#endif
            //SyncNetworkingHelpers.SendPacket(_connections.Last().Socket, new UploadFileMessage());
            _mainSocket.BeginAccept(OnNewConnection, this);
        }
        ...
        private void HandleConnectedSocket()
        {
            ...
            while (true)
            {
                ...
                {
                    _currentConnection = _connections[i];
                    if (_currentConnection.Socket.Available > 0)
                    {
                        try
                        {
                            var decodedPacked = SyncNetworkingHelpers.ReceivePacket(_connections[i].Socket, _connections[i].Buffer, _resolver);
                            if (!HandlePacket(decodedPacked))
                                toRemove.Add(_currentConnection);
                        }
...
#endif
                            if (_connections[i].Socket != null)
                            {
                                _connections[i].Socket.Dispose();
                            }
                            toRemove.Add(_currentConnection);
                        }
                    }
