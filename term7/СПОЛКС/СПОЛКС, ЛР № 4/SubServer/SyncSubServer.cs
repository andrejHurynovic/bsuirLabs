...
namespace SubServer
{
    internal class SyncSubServer
    {
        ...
        public void Start()
        {
            ...
            ConsoleHelpers.Log($"SubServer started on {_mainSocket.LocalEndPoint}", typeof(SyncSubServer).ToString(), true);
            ConsoleHelpers.Log($"Wait for new connection", typeof(SyncSubServer).ToString(), true);
#endif
            var handler = _mainSocket.Accept();
#if CONSOLE_LOGGING_ALL
            ConsoleHelpers.Log($"New connection: {handler.RemoteEndPoint}", typeof(SyncSubServer).ToString(), true);
#endif

            _currentConnection = new Connection(handler, BufferSize);
            while (true)
            {
                try
                {
                    var decodedPacket = SyncNetworkingHelpers.ReceivePacket(_currentConnection.Socket, _currentConnection.Buffer, _resolver);
                    if (!HandlePacket(decodedPacket))
                        return;
                }
                catch (Exception e)
                {
                    ConsoleHelpers.LogError($"Error with hanlding socket: {e.Message}", typeof(SyncSubServer).ToString(), true);
                    return;
                }
            }
        }
        ...
        {
#if CONSOLE_LOGGING_NOTIFIES || CONSOLE_LOGGING_ALL
            ConsoleHelpers.Log($"received packet: {decodedPacket.GetType().Name} (ID {decodedPacket.PacketID} NUM {decodedPacket.CurrentPacketNumber})", typeof(SyncSubServer).ToString(), true);
#endif
