...
{
    internal class SyncTcpClient : IDisposable
    {
        ...
        public SyncTcpClient(EndPoint endPoint)
        {
            _mainSocket = new Socket(AddressFamily.InterNetwork, SocketType.Stream, ProtocolType.Tcp);

            _mainSocket.SetSocketOption(SocketOptionLevel.Tcp, SocketOptionName.TcpKeepAliveInterval, 5);
            _mainSocket.SetSocketOption(SocketOptionLevel.Tcp, SocketOptionName.TcpKeepAliveRetryCount, 5);
            _mainSocket.SetSocketOption(SocketOptionLevel.Tcp, SocketOptionName.TcpKeepAliveTime, 10);
        ...
        public void Start()
        {
            while (true)
            {
                ...
                PacketBase packet = SyncNetworkingHelpers.ReceivePacket(_mainSocket, _buffer, _resolver);
                var startMessage = (StartMessage)packet;
                IPEndPoint temp = (IPEndPoint)_endPoint;
                _endPoint = new IPEndPoint(temp.Address, startMessage.Port);
                _mainSocket.Disconnect(false);
                ConsoleHelpers.Log("Disconnected from Gate-Server", typeof(SyncTcpClient).ToString(), true);
                ...
                if (TempFile.IsExist())
                {
                    ConsoleHelpers.LogWarning("Some file is not loaded! Trying to restore download!", typeof(SyncTcpClient).ToString(), true);
                    RestoringDownload();
                }
        ...
        {
            ConsoleHelpers.LogNotify("Some file is not loaded! Trying to restore download!");

            TempFile.GetInfo(out var allChunks, out var dowloadedChunks, out var fileKey, out string _);
            ...
            {
                ConsoleHelpers.LogSuccess("File downloaded! Open? (y/n)", typeof(SyncTcpClient).ToString(), true);
        ...
        private void ConnectToEP()
        {
#if CONSOLE_LOGGING_ALL
            ConsoleHelpers.Log($"Trying to connect to EP {_endPoint}", typeof(SyncTcpClient).ToString(), true);
#endif
            //try
            //{
            ...
            ////catch (Exception ex)
            ////{
            //    ConsoleHelpers.LogError($"Unable to connect to server. Error: {ex.Message}", typeof(SyncTcpClient).ToString(), true);
            //    throw;
            //}

#if CONSOLE_LOGGING_ALL
            ConsoleHelpers.LogSuccess($"Trying to connect to EP {_endPoint}", typeof(SyncTcpClient).ToString(), true);
#endif
        }
        ...
        private void Upload(string? fileName, bool newUpload = true)
        {
            ...
            while (true)
            {
                var decodedPacket = SyncNetworkingHelpers.ReceivePacket(_mainSocket, _buffer, _resolver);
                ...
                {
                    SyncNetworkingHelpers.SendPacket(_mainSocket, responsePacket);
                }
            }
        }
        ...
        {
            SyncNetworkingHelpers.SendPacket(_mainSocket, message);
        }

        /// <returns>response from server</returns>
        private T? RequestServer<T>(PacketBase request) where T : PacketBase
        {
            SyncNetworkingHelpers.SendPacket(_mainSocket, request);
            var raw = SyncNetworkingHelpers.ReceivePacket(_mainSocket, _buffer, _resolver);
