...
istream& operator >> (istream &inputStream, TouristBusinessman &touristBusinessman){
    ...
    touristBusinessman.pasportNumber = ExceptionEnter::EnterStringWithCheck(passportNumber, 9);
    
    int temp;
    BorderCrossing borderCrossing;

    do{
        cout << "Enter border crossing? (0/1)" << endl;
        cin >> temp;
    ...
    do{
        ...
        if(temp){
            cout << "Purchase address(max 16 symbols): ";
            purchaseAddress = ExceptionEnter::EnterStringWithCheck(purchaseAddressType, 16);
